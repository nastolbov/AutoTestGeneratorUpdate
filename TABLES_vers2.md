...
### 4.7. Описание компонентов системы

**Таблица 106. Описание компонентов системы**

| Наименование             | Назначение                                                                       | Входные данные                                                                          | Выходные данные                                                                            |
| ------------------------ | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `autotestgenerator.jar`  | Основное приложение: разбор XML-метаданных, генерация тестов, запуск и сбор отчётов | XML-файл метаданных E3Core; параметры (URL, логин, пароль, каталог, уровень тестов)     | Сгенерированный Maven-проект тестов; результаты прогона; HTML/CSV-отчёты; запись в `autotestgen.db` |
| `javafx-controls.jar`    | Библиотека графических элементов управления JavaFX                                | —                                                                                       | Классы UI-фреймворка: `Button`, `TextField`, `TableView`, `ListView`, `ComboBox` и др.    |
| `javafx-fxml.jar`        | Библиотека загрузки FXML-разметки и связывания с контроллером                     | Файл `main.fxml`                                                                        | Готовый граф объектов сцены с инжектированными `@FXML`-полями контроллера                  |
| `sqlite-jdbc.jar`        | JDBC-драйвер SQLite для доступа к локальной базе истории прогонов                  | JDBC-URL (`jdbc:sqlite:autotestgen.db`), SQL-запросы                                    | Объекты `java.sql.Connection` для выполнения INSERT/SELECT по `test_run` и `test_case`     |
| `java.xml.stream` *(встроен в JDK 17)* | StAX-парсер XML для поточного чтения метамодели E3Core                | XML-файл с namespace `NS_E`, `NS_E3`                                                    | События парсинга (START_ELEMENT, END_ELEMENT, атрибуты), читаемые `XMLStreamReader`         |
...
### 4.6. Спецификация программной документации и программного обеспечения (Прил. 3)

Детальная таблица для **Приложения 3** диплома. Формат соответствует табл. П3.1 из примера «Диплом_Столбов(1)»: для каждого `.java`-файла указано **Назначение**, **Входные параметры** и **Выходные параметры**, расширенное **Описание** (что класс делает в рамках программы), и **Состав** (от каких других модулей зависит).

**Таблица П3.1. Спецификация программной документации и программного обеспечения**
