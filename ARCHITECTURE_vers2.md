...
#### Описание методов пакета `ui`

**Класс `App`**

| Метод                                | Описание                                                                |
| ------------------------------------ | ----------------------------------------------------------------------- |
| `start(Stage primaryStage)`          | Загружает `main.fxml`, создаёт сцену 1000×700, показывает окно.         |
| `main(String[] args)`                | Точка входа. Вызывает `launch(args)` → JavaFX-runtime.                  |

**Класс `MainController`**

| Метод                          | Описание                                                                                       |
| ------------------------------ | ---------------------------------------------------------------------------------------------- |
| `initialize()`                 | Вызывается `FXMLLoader` после inject. Настраивает CellValueFactory колонок, дефолтные значения полей. |
| `onSelectXml()`                | Открывает `FileChooser`, выбранный путь записывает в `xmlPathField`.                          |
| `onSelectOutputDir()`          | Открывает `DirectoryChooser`, путь — в `outputDirField`.                                       |
| `onParse()`                    | Создаёт `XmlModelParser`, парсит XML, заполняет `entityListView`, делает `btnGenerate` активной. |
| `onGenerate()`                 | Создаёт `TestConfig`+`TestGenerator`, генерирует тест-проект, делает `btnRunTests`/`btnRunSelected` активными. |
| `onRunTests()`                 | Запускает `launchRun(null)` — все тесты.                                                       |
| `onRunSelected()`              | Открывает диалог выбора (чекбоксы сущностей × видов тестов), формирует Surefire-фильтр.        |
| `buildTestFilter(...)`         | Static. Из чекбоксов строит строку вида `Class1Test,Class2Test#m1+m2`.                          |
| `launchRun(testFilter)`        | Запускает `TestRunner` в `Task`, по завершении — `displayResults` + `reportDao.saveRun`.       |
| `onShowHistory()`              | Из `reportDao.getAllRuns()` выводит историю в `logArea`, последний прогон — в таблицу.         |
| `displayResults(result)`       | Заполняет `resultsTable` строками `TestCaseRow`, раскрашивает статусы, обновляет счётчики.     |
| `getXmlFileName()`             | Возвращает имя XML-файла (без пути) либо `"unknown.xml"`.                                       |
| `log(message)`                 | Дописывает строку в `logArea` через `Platform.runLater`.                                       |
| `showAlert(title, content)`    | Модальное `Alert.ERROR` через `Platform.runLater`.                                              |

**Класс `MainController.TestCaseRow`**

| Метод               | Описание                                                                          |
| ------------------- | --------------------------------------------------------------------------------- |
| Конструктор         | Принимает 5 строк (класс, метод, статус, длительность, сообщение).                |
| `getClassName()` и др. | Геттеры для `PropertyValueFactory` в `TableView`.                              |
