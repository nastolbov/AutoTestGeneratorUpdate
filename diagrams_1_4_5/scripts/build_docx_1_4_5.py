...
body("Логическая модель базы данных приведена на рисунке 10. В ней выделены две сущности: test_run — "
     "сведения об отдельном запуске набора автотестов (прогон), и test_case — результат выполнения "
     "отдельного тест-кейса в рамках прогона. Сущности связаны отношением «один ко многим» (1:N): "
     "один прогон содержит множество результатов тестов, и каждый результат принадлежит ровно одному "
     "прогону. Связь идентифицирующая и реализуется внешним ключом run_id в сущности test_case, "
     "ссылающимся на первичный ключ id сущности test_run.")
...
body("На логическом уровне для каждой сущности фиксируется состав атрибутов с указанием логического "
     "типа (целочисленный, строковый, дата-время, логический) и роли ключа — без привязки к "
     "конкретным типам SQLite, ограничениям и индексам, которые задаются в физической модели. Имена "
     "атрибутов соответствуют полям реальной базы данных. Состав атрибутов сущностей логической "
     "модели приведён в таблице 34.")
table_caption("Атрибуты сущностей логической модели")
grid(["Сущность", "Атрибут", "Логический тип", "Ключ / связь"],
     [["test_run", "id", "Целочисленный", "Первичный ключ (PK)"],
      ["", "run_date", "Дата-время", "Дата и время прогона"],
      ["", "xml_file", "Строковый", "Имя файла метамодели"],
      ["", "base_url", "Строковый", "Адрес тестируемого сайта"],
      ["", "total", "Целочисленный", "Всего тестов"],
      ["", "passed", "Целочисленный", "Успешно"],
      ["", "failed", "Целочисленный", "Провалено"],
      ["", "skipped", "Целочисленный", "Пропущено"],
      ["", "duration_ms", "Целочисленный", "Длительность прогона, мс"],
      ["test_case", "id", "Целочисленный", "Первичный ключ (PK)"],
      ["", "run_id", "Целочисленный", "Внешний ключ (FK) → test_run.id"],
      ["", "class_name", "Строковый", "Класс теста"],
      ["", "method_name", "Строковый", "Метод теста"],
      ["", "passed", "Логический", "Успешность (пройден/не пройден)"],
      ["", "failure_msg", "Строковый", "Сообщение об ошибке (может отсутствовать)"],
      ["", "duration_ms", "Целочисленный", "Длительность тест-кейса, мс"]],
     widths=[2.6, 3.2, 3.3, 7.4])
body("Таким образом, логическая модель отвечает на вопрос «какие данные и в каких связях хранятся», "
     "а физическая модель (рисунок 11, спецификации таблиц test_run и test_case) — «как именно эти "
     "данные реализованы в выбранной СУБД» с конкретными типами и ограничениями.")
