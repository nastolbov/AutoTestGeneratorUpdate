...
def package_section(pkg, X):
    ...
    body(f"Детальная диаграмма классов пакета «{title}» (рисунок {f5}) содержит поля и методы каждого класса с "
         f"указанием признака видимости (+ открытый, - закрытый, # защищённый, ~ пакетный); статические члены "
         f"подчёркнуты. Тривиальные геттеры и сеттеры полей для наглядности не приводятся (ни на диаграмме, ни в "
         f"таблицах). Состав полей и методов классов приведён в таблицах ниже.")
