...
OUT = os.path.join(ROOT, "Раздел_1.5_исправленный.docx")
...
def sig(m):
    ps = ", ".join((p["name"] + ": " + M.xref(p["type"], idx, m_pkg)) for p in m["params"])
    return m["name"] + "(" + ps + ")"
...
for pkg in M.PKG_ORDER:
    ...
    f1 = next_fig()
    body(f"Исходная диаграмма классов пакета «{title}» с составом классов и связями приведена на рисунке {f1}.")
    ...
    fn = {k: None for k in ("normal", "user", "system")}
    intro_nums = []
    for fk in ("normal", "user", "system"):
        if fk in flows:
            fn[fk] = next_fig(); intro_nums.append((fk, fn[fk]))
    ...
    body(f"Взаимодействие объектов классов пакета «{title}» показано диаграммами последовательности для "
         f"сценариев: {refs}. Управляющее воздействие поступает из края диаграммы; действующее лицо "
         f"«Пользователь» не изображается.")
    if note:
        body(note)
    ...
    body(f"Диаграмма кооперации (рисунок {f3}) представляет те же объекты, что и диаграммы последовательности "
         f"пакета, с нумерацией сообщений по сценариям (нормальный ход — 1, 2, …; прерывание пользователем — "
         f"п1, п2, …; прерывание системой — с1, с2, …). Участвующие объекты: {objs}.")
    ...
    f4 = next_fig()
    body(f"Уточнённая диаграмма классов пакета «{title}» (рисунок {f4}) фиксирует отношения между классами: "
         f"агрегацию (◇), композицию (◆), обобщение (▷), вложенность и кратности связей.")
    figure(f"cls_{pkg}_refined.png", f4, f"Уточнённая диаграмма классов пакета «{title}»")

    # .5 детальная + таблицы полей/методов
    heading(f"1.5.2.{X}.5 Разработка детальной диаграммы классов пакета «{title}»", 13)
    f5 = next_fig()
    body(f"Детальная диаграмма классов пакета «{title}» (рисунок {f5}) содержит полный состав полей и методов "
         f"каждого класса с указанием признака видимости (+ открытый, - закрытый, # защищённый, ~ пакетный); "
         f"статические члены подчёркнуты. Состав полей и методов классов приведён в таблицах ниже.")
    ...
    for r in types:
        ...
        if r["fields"]:
            ...
            frows = [[M.vis(f["access"]), f["name"], M.xref(f["type"], idx, pkg), M.describe_field(f)]
                     for f in r["fields"]]
            table(tnum, f"Поля класса {r['qualified']}", ["Дост.", "Имя", "Тип", "Описание"],
                  frows, [1.4, 3.6, 4.0, 7.5])
        if r["methods"]:
            tnum = next_tbl()
            ...
            for m in r["methods"]:
                ...
                mrows.append([M.vis(m["access"]), sig(m), ret, M.describe_method(pkg_qual, m)])
            table(tnum, f"Методы класса {r['qualified']}", ["Дост.", "Сигнатура", "Возвращает", "Описание"],
                  mrows, [1.4, 6.0, 3.1, 6.0])
