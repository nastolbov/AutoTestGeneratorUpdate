...
body(f"Для анализа структуры системы и оценки взаимодействия её модулей применяется карта Константайна "
     ...
     "и потоки данных. Вызовы модулей показаны сплошными стрелками, обращение к библиотекам – пунктиром "
     "(common coupling); у связей подписаны куплеты с направлением (↓ – передача в вызываемый модуль, "
     "↑ – возврат) и видом связи (○ – по данным, ● – по управлению).")
...
body("Сцепление определяет уровень зависимости между отдельными модулями системы. Наиболее рациональным "
     ...
     "по образцу (передаются составные объекты модели) и по данным; обращение к повторно используемым "
     "библиотекам отнесено к common coupling, а возврат вида сущности классификатором – к сцеплению "
     "по управлению. Оценка сцепления модулей приведена в таблице ниже.")
t_cou = next_tbl()
