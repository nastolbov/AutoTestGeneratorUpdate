...
# модульная структура
f1 = next_fig()
body(f"Модульная структура системы (дерево вызовов модулей) приведена на рисунке {f1}. На схеме показаны "
     f"функциональные модули системы ({n_func} модулей); повторно используемые библиотеки (StaxUtils, "
     "XmlNamespaces, DatabaseConnection, JavaFileWriter, Transliterator) на ней не приводятся — они вынесены "
     "на структурную карту Константайна.")
...
body(f"Для анализа модульной структуры и оценки взаимодействия модулей построена структурная карта "
     f"Константайна (рисунок {f2}). Вызовы модулей показаны сплошными стрелками, обращение к повторно "
     "используемым библиотекам — пунктиром (common coupling). У каждой связи подписаны куплеты — "
...
body("Преобладает сцепление по данным и по образцу, что соответствует рекомендациям структурного "
     ...
     "разбора метамодели (XmlModelParser) и вида классифицируемой сущности (EntityClassifier). Обращение к "
     "повторно используемым библиотекам отнесено к common coupling и на карте Константайна показано пунктиром.")
