...
body("Информационная система AutoTestGenerator построена по модульному принципу. Под модулем понимается "
     f"функциональный класс, выполняющий законченную задачу; всего система содержит {len(MM.MODULES)} "
     "функциональных модулей, распределённых по шести пакетам (ui, parser, model, generator, data, common). "
     "Классы-данные пакета model не являются модулями и рассматриваются как область данных «модель метаданных».")

# модульная структура
f1 = next_fig()
body(f"Модульная структура системы (дерево вызовов и использования модулей) приведена на рисунке {f1}. "
     "Библиотеки (повторно используемые модули без собственного управления) изображены двойной рамкой.")
figure("mod_struct.png", f1, "Модульная структура информационной системы")
t1 = next_tbl()
body(f"Краткая спецификация модулей системы приведена в таблице {t1}.")
table(t1, "Спецификация модулей системы", ["Модуль", "Пакет", "Назначение"],
      [[m[1], m[2], m[4]] for m in MM.MODULES], [4.2, 2.3, 10.0])

# карта Константайна
f2 = next_fig()
body(f"Для анализа модульной структуры и оценки взаимодействия модулей построена структурная карта "
     f"Константайна (рисунок {f2}). На ней показаны вызовы модулей, связи по данным (○) и по управлению (●), "
     "а также особые условия вызова: однократный (1), циклический (↺ «для каждой сущности») и условный (◇ "
     "«по виду сущности»). Библиотеки изображены двойной рамкой, области данных — овалом.")
...
body("Преобладает сцепление по данным и по образцу, что соответствует рекомендациям структурного "
     "проектирования; сцепление по управлению использовано только при передаче вида сущности классу "
     "EntityClassifier.")
