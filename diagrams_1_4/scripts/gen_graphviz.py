...
digraph SystemOps {{
  ...
  System [label="{{System|+ указатьФайлМетаданных(путь)\\l+ указатьКаталогГенерации(путь)\\l+ указатьАдресСайта(URL)\\l+ указатьУчётныеДанные(логин, пароль)\\l+ выбратьТипСайта(тип)\\l+ указатьПодсистему(имя)\\l+ разобратьМетаданные(файлXML)\\l+ просмотретьРезультатРазбора()\\l+ сгенерироватьАвтотесты(модель, каталог)\\l+ просмотретьСводкуГенерации()\\l+ запуститьАвтотесты(фильтрТестов)\\l+ просмотретьРезультатыПрогона()\\l+ сохранитьРезультаты(прогон)\\l+ просмотретьУведомлениеОСохранении()\\l+ запроситьИсториюПрогонов()\\l+ просмотретьДетализациюПрогона()\\l}}"];
}}
'''
