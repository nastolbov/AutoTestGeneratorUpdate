...
# 5 — Запустить выбранные тесты (extend)
make_ssd("ris_7_ssd_run_selected", "Диаграмма последовательности системы: «Запустить выбранные тесты»",
    ["запуститьВыбранныеТесты(сущности, видыТестов)"],
    notes=[(0, ["«extend» варианта", "«Запустить автотесты»"])])

# 6 — Сохранить результаты прогона (include)
make_ssd("ris_8_ssd_save", "Диаграмма последовательности системы: «Сохранить результаты прогона»",
    ["сохранитьРезультаты(прогон)"],
    notes=[(0, ["«include» из варианта", "«Запустить автотесты»"])])

# 7 — Просмотреть историю прогонов
make_ssd("ris_9_ssd_history", "Диаграмма последовательности системы: «Просмотреть историю прогонов»",
